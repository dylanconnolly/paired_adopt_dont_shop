...
RSpec.describe PetApplication, type: :model do
  ...
  describe '#pet_adopted?' do
    it "queries for all applications that pet exists and returns if any are true for adopted trait" do
      ...
      expect(pet_applications.pet_adopted?(pet_1)).to eq(false)

      pet_is_adopted = PetApplication.find_by(application_id: application_1.id)
      pet_is_adopted.update(approved: true)

      expect(pet_applications.pet_adopted?(pet_1)).to eq(true)
      expect(pet_applications.pet_adopted?(pet_2)).to eq(false)
    end
  end
end
